...
class VerticalViewPager : ViewGroup {
     ...
     * scrolled. See [android.support.v4.view.ViewPager.OnPageChangeListener].
     *
    ...
    /**
     * Set a [android.support.v4.view.ViewPager.PageTransformer] that will be called for each attached page whenever
    ...
    override fun getChildDrawingOrder(childCount: Int, i: Int): Int {
        val index =
            if (mDrawingOrder == DRAW_ORDER_REVERSE) childCount - 1 - i else i
    ...
    fun populate(newCurrentItem: Int = mCurItem) {
        ...
        if (curItem != null) {
            ...
            val topHeightNeeded: Float =
                if (clientHeight <= 0) 0f else 2f - curItem.heightFactor + paddingLeft.toFloat() / clientHeight.toFloat()
            ...
            if (child.visibility != GONE) {
                ...
                if (lp == null || !lp.isDecor) {
                    val heightSpec = MeasureSpec.makeMeasureSpec(
                        (childHeightSize * lp.heightFactor).toInt(), MeasureSpec.EXACTLY
                    )
    ...
    private fun pageScrolled(ypos: Int): Boolean {
        ...
        val pageOffset = (ypos.toFloat() / height - ii.offset) /
    ...
    override fun draw(canvas: Canvas) {
        ...
        if (overScrollMode == ViewCompat.OVER_SCROLL_ALWAYS ||
            overScrollMode == ViewCompat.OVER_SCROLL_IF_CONTENT_SCROLLS && adapter != null && adapter!!.count > 1
        ) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (mPageMargin > 0 && mMarginDrawable != null && mItems.size > 0 && adapter != null) {
            val scrollY = scrollY
            val height = height
            val marginOffset = mPageMargin.toFloat() / height
            var itemIndex = 0
            ...
            for (pos in firstPos until lastPos) {
                ...
                if (drawAt + mPageMargin > scrollY) {
                    mMarginDrawable!!.setBounds(
                        ...
                        mRightPageBounds, (drawAt + mPageMargin + 0.5f) as Int
                    )
        ...
        if (v is ViewGroup) {
            ...
            for (i in count - 1 downTo 0) {
                ...
                if (y + scrollY >= child.top && y + scrollY < child.bottom && x + scrollX >= child.left && x + scrollX < child.right &&
                    canScroll(
                        child, true, dy, x + scrollX - child.left,
                        y + scrollY - child.top
                    )
                ) {
                    return true
